...
function renderVehicles(vehicles) {
    ...
    grid.innerHTML = vehicles.map(vehicle => `
        <div class="group bg-white dark:bg-surface-dark rounded-xl overflow-hidden border border-slate-200 dark:border-metallic hover:shadow-2xl hover:shadow-primary/10 transition-all">
            <div class="relative h-64 overflow-hidden">
                <img alt="${vehicle.make} ${vehicle.model}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" src="${vehicle.image}"/>
                ...
                <div class="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                    ...
                    <div class="text-right">
                        <p class="text-white text-lg font-black tracking-tight">$${vehicle.price.toLocaleString()}</p>
                    </div>
                </div>
            </div>
            <div class="p-5 space-y-4">
                <div class="grid grid-cols-3 gap-4 py-2 border-b border-slate-100 dark:border-metallic">
                    ${vehicle.specs.map((spec, index) => `
                        <div class="text-center ${index === 1 ? 'border-x border-slate-100 dark:border-metallic' : ''}">
                            ...
                            <p class="text-sm font-black ${spec.label.includes('Reliability') || spec.label.includes('Safety') ? 'text-emerald-500' : ''}">${spec.value}</p>
                        </div>
                    `).join('')}
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button class="w-full bg-action-blue text-white py-3 rounded-lg text-xs font-black uppercase tracking-widest hover:brightness-110 shadow-lg shadow-action-blue/20 transition-all">
                        View Details
                    </button>
                    <button class="w-full border-2 border-slate-200 dark:border-metallic hover:border-primary py-3 rounded-lg text-xs font-black uppercase tracking-widest transition-all">
                        Test Drive
                    </button>
                </div>
            </div>
